/**
 * uniweb release — make a SYNCED site's current backend state live (CMS publish).
 *
 * Three "publish-ish" verbs, three jobs — don't conflate them:
 *   - `uniweb deploy`  hosts the CLI's FILE-BUILT payload (POST /dev/deploy).
 *   - `uniweb publish` catalogs a FOUNDATION (legacy; folding into `register`).
 *   - `uniweb release` publishes a SITE that already lives on the backend as a
 *     `@uniweb/site-content` entity (synced via `uniweb push`).
 *
 * `release` makes the site's CURRENT backend state live — including edits made
 * through the app since the last push. It does NOT push local files (run
 * `uniweb push` first if you want your local edits live, then `release`). The two
 ...
 * file-only site.
 *
 * Interim name: `release`. When `uniweb publish` (foundation catalog) folds into
 * `register`, this graduates to `uniweb publish` (the intuitive word for "make a
 * site live"). `publishSite()` in BackendClient is the stable plumbing underneath.
 *
 * Usage:
 *   uniweb release                       Publish the synced site's current state
 *   uniweb release --backend <url>       Override the backend origin
 *   uniweb release --token <bearer>      Auth bearer (skips `uniweb login`)
 *   uniweb release --dry-run             Resolve everything; POST nothing
 *
...
export async function release(args = []) {
  ...
  const siteDir = await resolveSiteDir(args, 'release')

  // The site-content uuid lives in site.yml::$uuid (written by `uniweb push`).
  // No uuid → the site was never synced; release has nothing to publish.
  ...
  if (!uuid) {
    ...
    say.dim('Run `uniweb push` first (release publishes the synced site), or use `uniweb deploy` for a file-only site.')
    return { exitCode: 1 }
  }
