/**
 * The ball's SHAPE is @uniweb/build's; the reader is here. Nothing in either
 * repo's own tests can see the seam between them.
 *
 * ⛔ It broke. `@uniweb/build` removed the ball's `search` key on 2026-08-01
 ...
 *
 * ⭐ This reads the REAL ball from the real builder. A hand-written fixture
...
import { assembleDataBall } from '@uniweb/build/site'
...
test('the ball a real build produces has exactly the keys the publish reader uses', async () => {
  const dist = distWithCollection()
  try {
    const ball = await assembleDataBall(dist, ['notes'])
    assert.ok(ball, 'a schema-less collection must produce a ball')
...
test('the ball is null when no collection is schema-less, so the reader is skipped', async () => {
  const dist = distWithCollection()
  try {
    // `notes` resolves a schema ⇒ it syncs as entities and contributes nothing.
    assert.equal(await assembleDataBall(dist, []), null)
  } finally {
    rmSync(dist, { recursive: true, force: true })
  }
})
