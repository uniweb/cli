...
import {
  ...
  hasShims,
  uploadRuntime
} from '../src/utils/runtime-upload.js'
...
function makeDist({ app = true, worker = true, shims = false } = {}) {
  ...
  if (app) {
    ...
    writeFileSync(
      join(distDir, 'app', 'assets', 'main.js.map'),
      '{"version":3}'
    ) // excluded
  }
  if (worker)
    writeFileSync(join(distDir, 'worker-runtime.js'), 'export const ssr=1')
  if (shims) {
    mkdirSync(join(distDir, 'shims'), { recursive: true })
    writeFileSync(join(distDir, 'shims', 'react.js'), 'export default {}')
    writeFileSync(
      join(distDir, 'shims', 'react-jsx-runtime.js'),
      'export const jsx=0'
    )
    writeFileSync(
      join(distDir, 'shims', 'uniweb-core.js'),
      'export const Uniweb=0'
    )
...
test('collectRuntimeFiles gathers dist/app/** + worker-runtime.js, excludes *.map', () => {
  ...
  try {
    const paths = collectRuntimeFiles(distDir)
      .map((f) => f.path)
      .sort()
    ...
    assert.ok(
      !files.some((f) => f.path.endsWith('.map')),
      'sourcemaps excluded'
    )
    assert.equal(
      files.find((f) => f.path === 'index.html').content_type,
      'text/html'
    )
    assert.equal(
      files.find((f) => f.path === 'manifest.json').sha256,
      sha('{"v":1}')
    )
    ...
    assert.equal(
      ...
      'manifest.json',
      'manifest uploads LAST (atomic version advertise)'
    )
  } finally {
...
test('collectRuntimeFiles gathers dist/shims/*.js as part of the ssr-edge set, excludes *.map', () => {
  ...
  try {
    ...
    assert.deepEqual(paths, [
      ...
      'shims/react.js',
      'shims/uniweb-core.js',
      'worker-runtime.js'
    ])
    assert.ok(
      !files.some((f) => f.path.endsWith('.map')),
      'shim sourcemaps excluded'
    )
    assert.equal(
      files.find((f) => f.path === 'shims/react.js').content_type,
      'text/javascript'
    )
    ...
    assert.equal(
      files[files.length - 1].path,
      'manifest.json',
      'manifest still LAST even with worker+shims appended after it'
    )
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
...
test('hasShims is false when shims/ is absent (incomplete ssr-edge set)', () => {
  ...
  try {
    ...
    assert.equal(
      hasShims(files),
      false,
      'worker present but shims missing → incomplete'
    )
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
...
test('uploadRuntime plans /dev/runtime, PUTs each (relative url resolved + auth), returns serveBase', async () => {
  ...
  try {
    const result = await uploadRuntime({
    ...
    assert.ok(
      puts.every((c) =>
        c.url.startsWith('http://localhost:8080/dev/runtime/blob/')
      ),
...
test('uploadRuntime throws with status 403 when the backend rejects a non-@std member', async () => {
  ...
  globalThis.fetch = async () => ({
    ...
    text: async () => 'not a member of @std'
  })
  try {
    await assert.rejects(
      ...
      (err) =>
        err.status === 403 &&
        /runtime plan rejected: HTTP 403/.test(err.message)
    )
  } finally {
    globalThis.fetch = realFetch
    rmSync(dir, { recursive: true, force: true })
...
test('presigned mode attaches no auth header on the PUT', async () => {
  ...
  try {
    ...
    assert.equal(
      sawAuth,
      false,
      'presigned PUT must not carry a foreign bearer'
    )
  } finally {
    globalThis.fetch = realFetch
    rmSync(dir, { recursive: true, force: true })
