...
function site({ uuid = null, git = false, remote = false } = {}) {
  ...
  writeFileSync(join(dir, 'site.yml'), `${uuid ? `$uuid: ${uuid}\n` : ''}name: S\nfoundation: '@a/base'\n`)
...
function capture(fn) {
  ...
  return fn().finally(() => { console.log = origLog; console.error = origErr }).then((r) => ({ r, out: lines.join('\n') }))
}
...
test('refresh names the sources it skipped rather than implying it checked everything', async () => {
  ...
  try {
    const { r, out } = await capture(() => refresh([], { resolveSiteDir: async () => dir, pull: async () => ({ exitCode: 0 }) }))
...
test('refresh skips git cleanly when the repo has no remote', { skip: !hasGit }, async () => {
...
test('refresh STOPS on a git failure instead of layering the backend on top', { skip: !hasGit }, async () => {
...
test('refresh --no-git reaches the backend step and reports the skip', async () => {
  ...
  try {
    const { out } = await capture(() => refresh(['--no-git'], { resolveSiteDir: async () => dir, pull: async () => ({ exitCode: 0 }) }))
...
test('refresh never pushes — no push verb is reachable from it', async () => {
  ...
  const src = readFileSync(new URL('../src/commands/refresh.js', import.meta.url), 'utf8')
  assert.ok(!/pushSyncPackages|publishSite|from '\.\/push\.js'|from '\.\/publish\.js'/.test(src))
})
...
test('refresh forwards --backend and its value to the delegated pull', async () => {
  ...
  try {
    let seen = null
    await capture(() => refresh(['--no-git', '--backend', 'http://127.0.0.1:9999'], {
      resolveSiteDir: async () => dir,
      pull: async (a) => { seen = a; return { exitCode: 0 } },
    }))
    assert.deepEqual(seen, ['--merge', '--backend', 'http://127.0.0.1:9999'])

    await capture(() => refresh(['--no-git', '--backend=http://x'], {
      resolveSiteDir: async () => dir,
      pull: async (a) => { seen = a; return { exitCode: 0 } },
    }))
...
test('refresh exits non-zero on conflicts, so `refresh && push` cannot ship markers', async () => {
  const dir = site({ uuid: 'SITE' })
  try {
    const { r, out } = await capture(() => refresh(['--no-git'], {
      resolveSiteDir: async () => dir,
      pull: async () => ({ exitCode: 1, merge: { clean: [], conflicted: ['pages/home/hero.md'], kept: [] } }),
    }))
    assert.equal(r.exitCode, 1)
