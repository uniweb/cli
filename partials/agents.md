...
⚠️ **The pin is a compatibility FLOOR, not a selector — and it is not read today.** It records what your build binds to; it does not choose the runtime a site runs, and it cannot: a site loads your foundation **plus any extensions**, each emitting its own pin, while a site has exactly **one** runtime. Pins are plural, the choice is singular. The runtime a site runs is selected by `site.yml::runtime`.

The pin's designed use is **validation** — checking that a selected runtime falls inside the compatible range of *every* foundation the site loads — and that is not implemented yet, so at present the pin is recorded and not consumed. Set `runtimePolicy` to state your intent for when it is; **don't design around it having an effect now**, and don't infer from a version in the file that anything acted on it.
