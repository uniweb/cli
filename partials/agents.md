...
**Runtime policy** (foundation authors only — sites don't set this). At build time a foundation records the `@uniweb/runtime` version it linked against into `dist/runtime-pin.json`, and carries `uniweb.runtimePolicy` alongside it when you set one: `exact` (stay on the recorded version), `auto-patch` (within `MAJOR.MINOR.x`), `auto-minor` (within `MAJOR.x.y`). **`auto-patch` is the one to prefer** while `@uniweb/*` is pre-1.0: a `0.x` **minor marks a breaking change**, and only a patch is guaranteed to leave the API your foundation links against untouched. Set `exact` only if you depend on undocumented runtime internals or have audited against one release. Site owners cannot override your choice.

> ⚠️ **Corrected 2026-08-11 — this paragraph previously said the runtime was "backwards-compatible at the minor level by convention" and recommended leaving the policy unset.** That was true of an older versioning convention and is not true now. While the framework is on `0.x`, npm's caret rule makes the minor slot the compatibility boundary (`^0.11.4` accepts `0.11.9`, refuses `0.12.0`), so breaking changes go there and `auto-minor` means *"accept breaking runtime updates"*. If you set a policy on an earlier reading of this text, `auto-patch` is the value you most likely wanted.
